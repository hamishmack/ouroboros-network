        ...
        checkBound (slotNo, hash) = case slotToEpochSlot dbm slotNo of
          EpochSlot epoch 1 ->
            case (Map.lookup (EpochSlot epoch 0, slotNo) blobs,
              ...
              (Just res1, Just res2)
                | let hash1 = either fst fst res1
                      hash2 = either fst fst res2
                -> Left $ WrongHashError slotNo hash (These hash1 hash2)
              (Just res1, Nothing)
                | let hash1 = either fst fst res1
                -> Left $ WrongHashError slotNo hash (This hash1)
              (Nothing, Just res2)
                | let hash2 = either fst fst res2
                -> Left $ WrongHashError slotNo hash (That hash2)
          epochSlot         ->
            case Map.lookup (epochSlot, slotNo) blobs of
              Nothing  -> Left $ EmptySlotError slotNo
              Just res
                  | hash' == hash -> return epochSlot
                  | otherwise     -> Left $ WrongHashError slotNo hash (That hash')
                where
