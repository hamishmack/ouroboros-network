...
preferAnchoredCandidate :: CanSelect p hdr
                        => NodeConfig p
                        -> AnchoredFragment hdr      -- ^ Our chain
                        -> AnchoredFragment hdr      -- ^ Candidate
                        -> Bool
preferAnchoredCandidate cfg = _
--- we don't have the anchor "header"
-- what to do if our chain is truncated, we have an empty fragment,
-- and we find a chain of the same length that we prefer?

--- precondition: both candidates preferred
