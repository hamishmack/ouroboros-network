...
getBlockImpl dbEnv getWhat slot =
    withOpenState dbEnv $ \_dbHasFS OpenState{..} -> do
      ...
      getEpochSlot _dbHasFS _dbHashInfo _dbErr getWhat epochSlot
  where
...
getEBBImpl dbEnv getWhat epoch =
    withOpenState dbEnv $ \_dbHasFS OpenState{..} -> do
      ...
      getEpochSlot _dbHasFS _dbHashInfo _dbErr getWhat (EpochSlot epoch 0)
  where
    ImmutableDBEnv { _dbEpochInfo, _dbErr, _dbHashInfo } = dbEnv

-- | Get the block (or just the header, depending on 'GetBlockOrHeader') and
-- hash corresponding to the given 'EpochSlot'.
--
...
getEpochSlot
  ...
  -> EpochSlot
  -> m (Maybe res)
getEpochSlot hasFS hashInfo err getWhat epochSlot =
        ...
        Just <$> case getWhat of
          ...
          GetBlock -> fmap (headerHash,) $
              ...
              withFile hasFS epochFile ReadMode $ \eHnd -> case blockSize of
                ...
                -- of the block. But it must be the last block, so we just
                -- read all bytes.
                Secondary.LastEntry      ->
...
appendEpochSlot hasFS err epochInfo hashInfo epochSlot blockOrEBB headerHash
    ...
    modify $ \st -> st
      { _currentEpochOffset     = Secondary.shiftBlockOffset _currentEpochOffset blockSize
...
startNewEpoch hasFS@HasFS{..} epochInfo = do
    ...
    let lastRelSlot     = maxRelativeSlot epochSize
