...
-- | TODO import from somewhere or share with other uses throughout the
-- codebase?
data Those a b
  = These a b
  | This  a
...
instance Bifunctor Those where
  bimap f g = \case
...
data WrongBoundError hash
  ...
  | WrongHashError SlotNo hash (Those hash hash)
    -- ^ The block and/or EBB in the given slot have a different hash.
    --
    -- The first @hash@ is the one given as bound. The @Those hash hash@
    -- contains the hash of the regular block ('That') in the slot and/or the
    -- hash of the EBB in the slot ('This').
