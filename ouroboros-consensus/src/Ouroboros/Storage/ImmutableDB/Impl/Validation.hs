...
validateAndReopen
  ...
  -> BaseIteratorID
  -> m (OpenState hash h)
validateAndReopen validateEnv@ValidateEnv { hasFS } valPol nextIteratorID = do
    validate validateEnv valPol >>= \case
      (epoch, TipGen) -> assert (epoch == 0)
...
-- | Validate epochs from oldest to newest, stop at the most recent epoch on
-- disk. During this validation, keep track of the last valid block in each
-- epoch. If at the end, that block is not in the last epoch on disk, remove
-- the epoch and index files after that epoch.
--
-- TODO stop when a block is missing, detect this by checking whether the
-- hashes line up
validateAllEpochs
  :: forall m hash h e. (IOLike m, Eq hash, HasCallStack)
  => ValidateEnv m hash h e
...
validateMostRecentEpoch
  ...
  -> m (EpochNo, ImmTipWithHash hash)
validateMostRecentEpoch validateEnv@ValidateEnv { hasFS } lastEpoch =
    go lastEpoch
  where
    go :: EpochNo -> m (EpochNo, ImmTipWithHash hash)
    go epoch =
      runExceptT (validateEpoch validateEnv ShouldNotBeFinalised epoch Nothing) >>= \case
        Right (Just validBlk) -> do
            -- Found a valid block, we can stop now.
            removeFilesStartingFrom hasFS (epoch + 1)
            return (epoch, Tip validBlk)
        _  -- Epoch should be truncated or is empty
          | epoch > 0 -> go (epoch - 1)
          | otherwise -> do
...
--   block.
--
-- * This function checks whether the first block in the epoch fits on the lat
--   block of the previous epoch by checking the hashes. If they do not fit,
--   this epoch is truncated and @()@ is thrown.
--
...
-- so we can determine whether the block or the checksum got corrupted.
validateEpoch
  :: forall m hash h e. (IOLike m, Eq hash, HasCallStack)
  => ValidateEnv m hash h e
  -> ShouldBeFinalised
  ...
  -> ExceptT () m (Maybe (BlockOrEBB, hash))
     ...
     -- block in the epoch. When the epoch file should be truncated, because
     -- it doesn't fit onto the previous epoch, @()@ is thrown.
validateEpoch ValidateEnv{..} shouldBeFinalised epoch mbPrevHash = do
    ...
    unless epochFileExists $ throwError ()
    ...
    lift $ do
      ...
      whenJust mbErr $ \(parseErr, endOfLastValidBlock) -> do
        ...
        traceWith tracer $ ValidatingEpochErrorParsing parseErr
      ...
      secondaryIndexFileMatches <- if secondaryIndexFileExists
        then do
          EH.try errLoad
            (Secondary.readAllEntries hasFS err hashInfo 0 epoch isEBB) <&> \case
              Left _                -> False
              -- TODO what if a hash or a checksum doesn't match? Who's
              -- speaking the truth? If the checksums match, then the hash of
              -- the epoch file is the right one. If the checksums don't
              -- match, we'd have to check the signature and whether the body
              -- of the block matches the body hash in the header to know
              -- which checksum is correct. If the signature check and so on
              -- pass, then the checksum of the block is correct, not the
              -- checksum in the secondary index entry. If they don't, the the
              -- block is corrupt and we should truncate it.
              --
              -- CURRENT APPROACH: we assume the contents of the epoch file
              -- are correct (however unrealistic that may be, as the chance
              -- of a bit flip in an epoch file is much greater than in an
              -- index file). We will do the right thing in the future.
              Right entriesFromFile -> map fst entriesFromFile == entries
        else return False
      ...
      primaryIndexFileMatches <- if primaryIndexFileExists
        then do
          EH.try errLoad (Primary.load hasFS err epoch) <&> \case
            Left _                     -> False
            Right primaryIndexFromFile -> primaryIndexFromFile == primaryIndex
        else return False
  ...
  where
    ...
    go nextExpectedRelSlot lastSecondaryOffset = \case
      [] -> case shouldBeFinalised of
        ShouldNotBeFinalised        -> []
        ShouldBeFinalised epochSize ->
            Primary.backfill (succ finalRelSlot) nextExpectedRelSlot
              lastSecondaryOffset
          where
            finalRelSlot = maxRelativeSlot epochSize
