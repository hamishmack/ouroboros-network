...
mkOpenState hasFS@HasFS{..} epoch nextIteratorID tip existing =
    ...
    withHandle     (hOpen (renderFile "epoch" epoch)     appendMode) $ \eHnd ->
      withHandle   (Primary.open hasFS epoch existing)               $ \pHnd ->
        withHandle (hOpen (renderFile "secondary" epoch) appendMode) $ \sHnd -> do
  ...
  where
    ...
    withHandle :: m (Handle h) -> (Handle h -> m a) -> m a
    withHandle open k = open >>= \h ->
